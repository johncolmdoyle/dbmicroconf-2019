module "ec2" {
  ...
  user_data = <<-EOF
              ...
              aws s3 sync s3://silbo-bastion-sm-public-keys/ ~/users_public_keys
...
module "bastion_sg" {
  ...
  name        = "bastion-sm-sg"
  description = "Security group for Bastion and Secrets Manager"
  vpc_id      = "${var.vpc_id}"
...
resource "aws_iam_role_policy" "s3_readonly_policy" {
  name = "microconf_sm_s3_readonly-policy"
    ...
    "Statement": [
        {
            ...
            "Resource": ["arn:aws:s3:::silbo-bastion-sm-public-keys","arn:aws:s3:::silbo-bastion-sm-public-keys/*"]
        }
    ]
}
EOF
