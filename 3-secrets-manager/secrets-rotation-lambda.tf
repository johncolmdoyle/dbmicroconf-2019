resource "aws_iam_role" "secrets_rotation_role" {
  name = "microconf_secrets_rotation_role"
...
resource "aws_iam_policy_attachment" "secrets_rotation_policy_attach" {
  name       = "microconf_secrets_rotation_policy_attach"
  roles      = ["${aws_iam_role.secrets_rotation_role.name}"]
...
resource "aws_lambda_function" "secrets_rotation" {
  ...
  function_name = "microconf_secrets_rotation"
...
resource "aws_secretsmanager_secret" "microconf_secrets_manager" {
  name                = "microconf_secrets_manager_user"
...
output "secret_id" {
  value = "${aws_secretsmanager_secret.microconf_secrets_manager.id}"
}
