module "db" {
  ...
  identifier = "microconf-iam-authentication"
  ...
  final_snapshot_identifier = "microconfdb-iam-authentication"
}
...
module "postgres_sg" {
  ...
  name        = "postgres-iam-sg"
  description = "Security group for Postgres Database with iam authentictaion"
  vpc_id      = "${var.vpc_id}"
